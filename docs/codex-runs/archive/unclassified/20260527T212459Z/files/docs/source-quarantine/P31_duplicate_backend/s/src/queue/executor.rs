...
async fn process_job(
    ...
    let img_ref = history.image_filenames.last()
    ...
    let local_filename = storage::generate_filename();
    {
        ...
        tokio::task::spawn_blocking(move || {
            storage::save_image_from_bytes_with_config(&config_clone, &bytes_clone, &filename_clone)
        })
        .await
    ...
    {
        let conn = state.db.lock().map_err(|e| anyhow::anyhow!("{}", e))?;
        if db::queue::is_job_cancelled(&conn, &job.id).unwrap_or(false) {
            // Clean up the file we just saved
            let config = state.config.read().map_err(|e| anyhow::anyhow!("{}", e))?;
            let _ = storage::delete_image_files_for(&config, &local_filename);
            anyhow::bail!("Job cancelled by user");
        }
    }
...
fn build_generation_request(job: &crate::types::queue::QueueJob) -> Result<GenerationRequest> {
    ...
    let checkpoint = settings
        ...
        .filter(|s| !s.is_empty())
        .map(|s| s.to_string())
        .context("Job is missing a checkpoint. Please select a checkpoint before queueing.")?;
