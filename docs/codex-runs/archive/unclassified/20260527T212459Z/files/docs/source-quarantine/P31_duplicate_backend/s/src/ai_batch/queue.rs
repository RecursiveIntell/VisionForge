...
impl AiBatchQueue {
    ...
    fn reorder_queued_jobs(jobs: &mut Vec<BatchJob>) {
        ...
        let mut queued_jobs: Vec<BatchJob> = queued_indices
            .iter()
            .map(|&i| jobs[i].clone())
            .collect();
        ...
        if original_order != new_order {
            for job in &mut queued_jobs {
                job.reordered = true;
                job.reorder_note = Some(
                    "Reordered: grouping by model to minimize GPU swaps".to_string(),
                );
            }
            for (slot_idx, job) in queued_indices.iter().zip(queued_jobs.into_iter()) {
                jobs[*slot_idx] = job;
            }
        }
    }
        ...
        if let Some(job) = jobs.iter_mut().find(|j| j.id == job_id) {
            if let Some(item) = job.items.iter_mut().find(|i| i.image_id == image_id) {
                ...
                if should_record {
                    ...
                    if let Ok(mut eta) = self.eta_data.lock() {
    ...
    pub fn mark_completed(
        ...
        if let Some(job) = jobs.iter_mut().find(|j| j.id == job_id) {
            ...
            let succeeded = job.items.iter().filter(|i| i.status == BatchItemStatus::Completed).count();
            let skipped = job.items.iter().filter(|i| i.status == BatchItemStatus::Cancelled).count();
            ...
            let avg_ms = if processed > 0 { total_ms / processed as u64 } else { 0 };
    ...
    pub fn cancel_job(&self, job_id: &str) -> anyhow::Result<()> {
        ...
        if let Some(job) = jobs.iter_mut().find(|j| j.id == job_id) {
            ...
            let any_running = job.items.iter().any(|i| i.status == BatchItemStatus::Running);
            if !any_running {
    ...
    pub fn get_job(&self, job_id: &str) -> Option<BatchJob> {
        self.jobs.lock().ok()?.iter().find(|j| j.id == job_id).cloned()
    }
    ...
    pub fn estimate_remaining_ms(&self, job_id: &str) -> Option<u64> {
        ...
        if has_data { Some(total_estimate) } else { None }
    }
    ...
    pub fn queued_count(&self) -> usize {
        self.jobs
            .lock()
            .map(|j| j.iter().filter(|job| job.status == BatchJobStatus::Queued).count())
            .unwrap_or(0)
    }
}
...
mod tests {
    ...
    fn test_cancel_job() {
        ...
        assert!(job.items.iter().all(|i| i.status == BatchItemStatus::Cancelled));
    }
