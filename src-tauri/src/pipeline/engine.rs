...
pub async fn run_pipeline(
    ...
    let composed = if stages_enabled[1] {
        let mut composed_descs = Vec::new();
        let mut best_composer: Option<ComposerOutput> = None;
        ...
        for (i, concept) in concepts.iter().enumerate() {
            ...
            // Keep the last one as the representative output for lineage
            best_composer = Some(output);
        }

        // Store the first composer output for lineage (will update with top-ranked later)
        result_stages.composer = best_composer;
        composed_descs
    } else {
    ...
    // Update composer output to reflect the selected concept
    if stages_enabled[1] && result_stages.composer.is_some() {
        // Re-run or retrieve the composer output for the selected concept
        if let Some(ref mut composer) = result_stages.composer {
            composer.input_concept_index = selected_index;
            if let Some(desc) = composed.get(selected_index) {
                composer.output = desc.clone();
            }
            if let Some(concept) = concepts.get(selected_index) {
                composer.input = concept.clone();
            }
        }
    }
